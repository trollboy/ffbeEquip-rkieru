...
var searchClickBehavior = ClickBehaviors.EQUIP,
    currentItemSlot,
    searchableEspers,
    dataLoadedFromHash = false,
    conciseView = true,
    progressElement,
    progress,
    defaultItemsToExclude = ["409009000"],
    itemsToExclude = defaultItemsToExclude.slice(), // Ring of Dominion
    itemsToInclude = [],
    itemPool,
    running = false,
    workers = [],
    ...
    processedCount = 0,
    typeCombinationsCount,
    remainingTypeCombinations,
    dataStorage = new DataStorage(),
    bestiary,
    typeCombinationChunckSizeDefault = 2,
    typeCombinationChunckSize = typeCombinationChunckSizeDefault,
    goalVariation = "min",
    initialPinnedWeapons,
    currentEnchantmentItem,
    savedBuilds = null,
    currentSavedBuildIndex = -1,
    secondaryOptimization = false,
    secondaryOptimizationFixedItemSave,
    secondaryOptimizationFormulaSave,
    defaultWeaponEnhancement = [],
...
/* Here lies a bajillion IDs and class names so we don't have to reference them
 * Individually a hundred thousand or more times ^_^ --------------------------------------------- */

var alert_dataCap          = '#alert_dataCap',
    alert_noUnitSelect     = '#alert_noUnitSelect',
    alert_unitMonsterBreak = '#alert_unitMonsterBreak';

var trig_selectUnit        = '#trig_selectUnit',
    trig_selectGoal        = '#trig_selectGoal',
    trig_selectMulticast   = '#multicast-',
    trig_selectParam       = '#trig_selectParam',
    trig_selectParamBtn    = '#trig_selectParamBtn',
    trig_filter7Star       = 'trig_filter7Star',
    trig_tabSelectUnit     = '#tab_selectUnit',
    trig_tabParamChallenge = '#tab_paramChallenge',
    trig_unitSwitchPot     = '#trig_unitSwitchPot';

var tog_unitConfigDisplay  = '.tog_unitConfigDisplay',
    tog_btnSaveTeam        = '#tog_btnSaveTeam',
    tog_btnSaveTeamAs      = '#tog_btnSaveTeamAs',
    tog_btnOpenTeam        = '#tog_btnOpenTeam',
    tog_btnExportBattle    = '#tog_btnExportBattle';

var input_ChainMulti       = '.input_ChainMulti',
    input_unitAilImmune    = '#input_unitAilImmune',
    input_unitElDamage     = '#input_unitElDamage',
    input_unitImbue        = '#input_unitImbue';

var con_tabSelectUnit      = '#con_selectUnit',
    con_tabParamChallenge  = '#con_paramChallenge',
    con_paramChallengeData = '.con_paramChallengeData',
    con_selectGoal         = '.con_selectGoal',
    con_selectMulticast    = '#con_selectMulticast',
    con_customGoal         = '#con_customGoal',
    con_customGoalFormula  = '#con_customGoalFormula',
    con_selectSkillStack   = '.con_selectSkillStack',
    con_skillSimple        = '.con_skillSimple',
    con_skillSimpleDisplay = '#con_skillSimpleDisplay',

    con_unitConfig         = '.con_unitConfig',
    con_unitIconMain       = '#con_unitIconMain',
    con_unitIconRarity     = '.con_unitIconRarity',
    con_unitLink           = '#con_unitLink',
    con_unitEquipment      = '.con_unitEquipment',
    con_unitExAwakening    = '#con_unitExAwakening',
    con_unitEnhance        = '#con_unitEnhance',
    con_unitStats          = '#con_unitStats',
    con_unitKiller         = '#con_unitKiller',
    con_unitImbue          = '.con_unitImbue',
    con_unitMonsterList    = '#con_unitMonsterList',

    con_btnSaveBuild       = '#con_btnSaveBuild',
    con_unitBuildDisplay   = '#con_unitBuildDisplay',
    con_buildLinks         = '#con_buildLinks',
    con_buildWelcomeText   = '#con_buildWelcomeText';
...
function build() {
  ...
  setDataState('.con_unitBuildTry', 'shown');
...
function optimize() {
  ...
  var forceDoubleHand       = $("#forceDoublehand").prop('checked'),
      forceDualWield        = $("#forceDualWield").prop('checked'),
      tryEquipSources       = $("#tryEquipsources").prop('checked'),
      useNewJpDamageFormula = $("#useNewJpDamageFormula").prop('checked');

  prepareDataStorage();

  var espersToSend = {},
      esperNames   = Object.keys(espersByName);
...
function readGoal(index = currentUnitIndex) {
  ...
  if (customFormula) {
    ...
    var goalValue = $(trig_selectGoal).val();
           ...
           for (var effectIndex = passive.effects.length; effectIndex--;) {
             ...
             if (effect && effect.replaceLb) {
                    ...
                    for (var i = 0, len = multicastEffect.time; i < len; i++) {
                        var skillChoiceValue = $(trig_selectMulticast + i).val();
    ...
    $(con_selectSkillStack).toggleClass("hidden", !hasStack(builds[index].formula));

    setDataState(con_unitImbue, 'hidden');
    ...
    if (builds[index].unit &&
        ...
        setDataState(con_unitImbue, 'shown');
    }
...
function getItemLine(index, short = false) {
    ...
    if (index >= 0 && builds[currentUnitIndex].fixedItems[index]) {
        ...
        if (weaponList.includes(item.type)) {
            html += '<img class="itemEnchantmentButton" title="Modify this weapon enchantment" src="/assets/media/icons/dwarf.png" onclick="currentItemSlot = ' + index + ';selectEnchantement(getRawItemForEnhancements(builds[currentUnitIndex].fixedItems[' + index + ']))" />';
        }
        html += '</div>';
    } else if (!item) {
        ...
        if (weaponList.includes(item.type)) {
            html += '<img class="itemEnchantmentButton" title="Modify this weapon enchantment" src="/assets/media/icons/dwarf.png" onclick="currentItemSlot = ' + index + ';selectEnchantement(getRawItemForEnhancements(builds[currentUnitIndex].build[' + index + ']))" />';
        }
        html += '</div>';
    } else {
...
function populateUnitSelect() {
    ...
    selector.on('select2:open', function (e) {
        $('<div class="custom-control custom-switch mx-2 mb-1"><input type="checkbox" class="custom-control-input" id="' + trig_filter7Star + '" class="checkbox" ' + (displayOnly7StarsUnits ? 'checked' : '') + '><label class="custom-control-label" for="' + trig_filter7Star + '">Only 7★ units</label></div>')
...
function refreshUnitSelect() {
    var selector = $(trig_selectUnit);
    $('#' + trig_filter7Star).prop("checked", displayOnly7StarsUnits);
    selector.html(getUnitSelectOptions());
...
function selectUnitDropdownWithoutNotify(unitId) {
    ...
    $(trig_selectUnit).val(unitId).trigger('change.select2');
}

async function selectUnit(unitId) {
    $(trig_selectUnit).val(unitId);
    $(trig_selectUnit).trigger('change');
    await onUnitChange();
}
...
function goalSelectTemplate(state) {
    ...
    var html;
    if (state.id == "LB") {
        html = '<img class="selectIcon" src="/assets/media/icons/lb.png"> ' + state.text;
        var formula = formulaFromSkill(unitWithSkills.lb);
        if (formula.notSupported) {
            html = html.replace("- Not supported yet", "<span class='badge badge-skill ml-auto notSupportedTag'>Not yet</span>");
        } else {
            html = html.replace("(Limit Burst)", "<span class='badge badge-skill ml-auto lbTag'>Limit burst</span>");
        }
        ...
        if (skill) {
            html = '<img class="selectIcon" src="/assets/media/items/' + skill.icon + '"> ' + state.text;
            ...
            if (formula.notSupported) {
                ...
                if (skill.magic) {
                    html += "<span class='badge badge-skill ml-auto magicTag'>magic</span>";
                } else {
                    html += "<span class='badge badge-skill ml-auto skillTag'>skill</span>";
                }
            }
        } else {
            html = state.text;
        ...
        if (skill) {
            html = '<img class="selectIcon" src="/assets/media/items/' + skill.icon + '"><span>' + state.text + "</span><span class='badge badge-skill ml-auto multicastTag'>multicast</span>";
        } else {
            html = state.text;
        }
    } else {
        ...
        switch (state.id) {
            ...
            case "summonerSkill":
                html += "<span class='badge badge-skill ml-auto damageTag'>damage</span>";
                break;
            case "physicaleHp":
            case "magicaleHp":
            case "atk":
            ...
            case "mpRefresh":
            case "heal":
                html += "<span class='badge badge-skill ml-auto statTag'>stat</span>";
                break;
            default:
                break;
        }
    }
    return $('<div class="d-flex align-items-center selectSkillItem">' + html + '</div>');
}

function onUnitChange() {
  return new Promise(async resolve => {

    if ($(trig_selectUnit).find(':selected').length > 0) {
      var unitId = $(trig_selectUnit).val();
      ...
      if (selectedUnitData) {
        let unitWithSkills = await ensureInitUnitWithSkills(unitWithSkillsId);
                $("#unitTabs .tab_" + currentUnitIndex + " a").html('<img src="/assets/media/units/unit_icon_' + iconId + '.png" class="unitTab-img">' + selectedUnitData.name);
                ...
                // Change Visibility
                setDataState(con_unitBuildDisplay, 'shown');
                setDataState(tog_unitConfigDisplay, 'shown');

                //$(tog_unitConfigDisplay).toggleClass('show').attr('aria-expanded', true);

                setDataState(con_selectGoal, 'shown');
                setDataState(con_skillSimple, 'shown');
                setDataState(con_buildWelcomeText, 'hidden');

                if (unitId === '777700004') {
                  setDataState(tog_btnExportBattle, 'shown');
                } else {
                  setDataState(tog_btnExportBattle, 'shown');
                }

                if (itemInventory) {
                  setDataState(tog_btnSaveTeam, 'shown');
                }
                ...
                //setDataState(con_selectGoal, 'hidden');
                //setDataState(con_skillSimple, 'hidden');
            }
...
function updateGoal() {
    ...
    var choiceSelect = $(trig_selectGoal);
    ...
    if (builds[currentUnitIndex].unit) {
        ...
        if (selectedSkill) {
            ...
            if (chainMultiplier) {
                $(input_ChainMulti).val(chainMultiplier);
            }
        } else {
        ...
        if (builds[currentUnitIndex].goal == 'custom' && !isSimpleFormula(builds[currentUnitIndex]._formula)) {
            setDataState(con_selectGoal, 'hidden');
            $(con_customGoalFormula).text(formulaToString(builds[currentUnitIndex]._formula));
    ...
    choiceSelect.select2({
        placeholder: 'Select a Build Priority...',
        theme: 'bootstrap4',
...
function displayUnitEnhancements() {
    ...
    if (builds[currentUnitIndex].unit && builds[currentUnitIndex].unit.enhancements) {
        var html = '<label class="col-form-label col-2">Enhance</label><div class="col">';
        for (var i = 0, len = builds[currentUnitIndex].unit.enhancements.length; i < len; i++) {
            ...
            html += '<div class="unitEnhancement"><select class="custom-select custom-select-sm mb-1" onchange="onUnitChange().then(() => {});" id="enhancement_' + i + '">';
...
function updateUnitStats() {
    ...
    if (builds[currentUnitIndex].unit) {
        ...
        builds[currentUnitIndex].unit.equip.forEach(equip => {
            let equipImg = $(con_unitEquipment).find('.img-equipment.' + equip);
    ...
    if (builds[currentUnitIndex].unit) {
        $(con_unitIconMain).prop("src", "/assets/media/units/unit_icon_" + builds[currentUnitIndex].unit.id + ".png");
        $(con_tabSelectUnit).removeClass("hidden");
    } else {
        $(con_tabSelectUnit).addClass("hidden");
    }
...
function loadBuild(buildIndex) {
    ...
    $(input_unitElDamage + " div.elements label").removeClass("active");
    if (build.innateElements) {
        for (var i in build.innateElements) {
            $(input_unitElDamage + " div.elements label:has(input[value=" + build.innateElements[i] + "])").addClass("active");
        }
    }
...
function addNewUnit(focusUnitSelect = true) {
    ...
    var newTab = $("<li class='nav-item active tab_" + newId + "'><a href='#' class='nav-link'>Select Unit</a><button type='button' class='closeTab' onclick=\"closeTab()\"><span class=\"fas fa-times\"></span></button></li>");
    ...
    if (focusUnitSelect) {
        $(trig_selectUnit).select2('open');
    }
}

function selectUnitTab(index) {
    $("#unitTabs .nav-item, #unitTabs .nav-link").removeClass("active");
    $("#unitTabs .nav-item.tab_" + index).addClass("active");
    $("#unitTabs .nav-item.tab_" + index + ' .nav-link').addClass("active");
    loadBuild(index);
}
...
var displayUnitRarity = function(unit) {
  var rarityWrapper = $(con_unitIconRarity);
  ...
  if (unit) {
    ...
    if (rarity == 'NV') {
      rarityWrapper.append('<img src="/assets/media/icons/NV.png">');
    } else {
      for (var i = 1; i < 8; i++) {
        if (i <= rarity) {
          rarityWrapper.append('<span class="unitRarity-star"></span>');
        } else {
          rarityWrapper.append('<span class="unitRarity-star inactive"></span>');
        }
      }
    }
...
function manageMulticast(selectedSkills) {
    $(con_selectMulticast + ' select').addClass("hidden");
    for (var i = 0; i < 5; i++) {
        var select = $(trig_selectMulticast + i);
...
function getItemHtml(item) {
  ...
  if (item) {
    var excluded = itemsToExclude.includes(item.id),
        included = itemsToInclude.includes(item.id),
        enhancementString = "null",
        enhancementClass  = '',
        onClickBehavior   = '"',
        clickBehaviorLink = '',
        enhanceLink       = '';
    ...
    if (searchClickBehavior == ClickBehaviors.IGNORE) {
      ...
      onClickBehavior = '" onclick="toggleExclusionFromSearch(\'' + item.id + '\');"';
      $('#fixItemModal').addClass('exclusion');
    } else if (searchClickBehavior == ClickBehaviors.INCLUDE) {
      onClickBehavior = '" onclick="toggleInclusionInSearch(\'' + item.id + '\');"';
      $('#fixItemModal').addClass('inclusion');
    } else {
      onClickBehavior = '" onclick="fixItem(\'' + item.id + '\', ' + currentItemSlot + ', ' + enhancementString + ')"';
    }

    if (searchClickBehavior == ClickBehaviors.INCLUDE) {
      clickBehaviorLink= '<div class="p-1 include">' + getItemInclusionLink(item.id, included) + '</div>';
    } else {
      clickBehaviorLink= '<div class="p-1 exclude">' + getItemExclusionLink(item.id, excluded) + '</div>';
    }

    if (searchClickBehavior != ClickBehaviors.EXCLUDE && searchClickBehavior != ClickBehaviors.INCLUDE) {
      enhanceLink = "<div class='enchantment desktop'>" + getItemEnhancementLink(item) + "</div>";
    }

    var html  = '<div class="col-12 col-md-6 mb-3 selectable itemWrapper ' + enhancementClass + onClickBehavior + '>';
        html += '  <div class="border rounded p-3 d-flex align-items-center">';
        html += '    <div class="flex-shrink">' + clickBehaviorLink + '</div>';
        html += '    <div class="flex-fill"><div class="form-row">' + displayItemLine(item) + enhanceLink + '</div></div>';
        html += '  </div>';
        html += '</div>';
...
function getItemEnhancementLink(item) {
    ...
    if (weaponList.includes(item.type)) {
        html += '<div class="enchantment"><img src="/assets/media/icons/dwarf.png" onclick="event.stopPropagation();selectEnchantedItem(\'' + item.id + '\')">';
...
async function loadStateHashAndBuild(data, importMode = false) {
    ...
    $(trig_selectGoal + ' option').prop("selected", false);
...
function getSavedTeamList() {
    ...
    for (var i = savedBuilds.teams.length - 1; i >= 0; i--) {
        ...
        for (var j = 0, lenj = savedBuilds.teams[i].team.units.length; j < lenj; j++) {
            html += '<img class="unit" src="/assets/media/units/unit_icon_' + savedBuilds.teams[i].team.units[j].id + '.png">';
        }
        html += '</div></div><div>' +
...
function onFeatureSelect(selectedFeature) {
  if (selectedFeature === 'buildAUnit') {
    ...
    setDataState(con_btnSaveBuild, 'hidden');

    if (itemInventory) {
      setDataState(alert_paramNotAvailable, 'hidden');
      setDataState(con_paramChallengeData, 'shown');
      setDataState(con_buildWelcomeText, 'hidden');

    } else {
      setDataState(alert_paramNotAvailable, 'shown');
      setDataState(con_paramChallengeData, 'hidden');
    }
...
function switchDisplayKillerBuffs() {
    $('.killerBuffs .physical').toggleClass('hidden');
    $('.killerBuffs .magical').toggleClass('hidden');
    $('.killerBuffsSummary').toggleClass('hidden');
}
...
function updateKillerBuffSummary() {
    ...
    $('.killerBuffsSummary .physical').html(killerHtml.physical);
    $('.killerBuffsSummary .magical').html(killerHtml.magical);
}
...
function startPage() {
    ...
    $(trig_selectGoal).on("select2:select", function() {
        customFormula = null;
        manageMulticast();
        onGoalChange();
    });
    ...
    // Elements
    addIconChoicesTo(input_unitImbue, elementList, "checkbox", "element", ucFirst);
    addIconChoicesTo(input_unitElDamage, elementList.concat(['noElement']), "checkbox", "element", function(v) {
      if (v == "noElement") {
    ...
    addIconChoicesTo(input_unitAilImmune, ailmentList.slice(0, 9), "checkbox", "ailment", ucFirst);
    ...
    for (let statIndex = baseStats.length; statIndex--;) {
        ...
        $(".unitStats .stat." + baseStats[statIndex] + " .pots .pots-plus").click(function(stat) {
            if (builds[currentUnitIndex].unit) {
    ...
    $(con_selectMulticast + " select").change(function() {customFormula = null; logCurrentBuild();});
...
/* Output those snazzy weapon and armor icons ---------------------------------0------------------ */

function populateUnitEquip() {
    var tarWeapons = con_unitEquipment + " .weapons",
        tarArmor   = con_unitEquipment + " .armors",
        boostIcon = '<i class="fas fa-arrow-alt-circle-up boostIcon"></i>',
        banIcon = '<i class="fas fa-ban banIcon"></i>';
    ...
    for (var key in weaponList) {
        $(tarWeapons).append('<div class="img-equipment img-md '+weaponList[key]+' notEquipable" onclick="toogleEquipableType(\'' + weaponList[key] + '\');">' + boostIcon + banIcon + '</div>');
    }

    // Build Armor
    for (var key in shieldList) {
        $(tarArmor).append('<div class="img-equipment img-md '+shieldList[key]+' notEquipable" onclick="toogleEquipableType(\'' + shieldList[key] + '\');">' + boostIcon + banIcon + '</div>');
    }

    for (var key in headList) {
        $(tarArmor).append('<div class="img-equipment img-md '+headList[key]+' notEquipable" onclick="toogleEquipableType(\'' + headList[key] + '\');">' + boostIcon + banIcon + '</div>');
    }

    for (var key in bodyList) {
        $(tarArmor).append('<div class="img-equipment img-md '+bodyList[key]+' notEquipable" onclick="toogleEquipableType(\'' + bodyList[key] + '\');">' + boostIcon + banIcon + '</div>');
    }

    if (builds[currentUnitIndex]) {
        builds[currentUnitIndex].bannedEquipableTypes.forEach(type => $(con_unitEquipment).find('.img-equipment ' + type).addClass("banned"));
    }
}

function toogleEquipableType(equipableType) {
    builds[currentUnitIndex].toogleEquipableType(equipableType);
    $(con_unitEquipment).find('.img-equipment.' + equipableType).toggleClass("banned");
}
...
function populateItemType(equip) {
    ...
    target.html("");
    if (equip.length > 1) {
        target.append("<li class='all sort-type'><a onclick='selectSearchType(" + JSON.stringify(equip) + ");updateSearchResult();'><img src='/assets/media/icons/all.png'/></a></li>");
    }
	for (var key in equip) {
        target.append('<li class="' + equip[key] + ' sort-type"><a onclick="selectSearchType([\'' + equip[key] + '\']);updateSearchResult();">'+
                      '<div class="img-equipment ' + equip[key] + '"></div>'+
                      '</a></li>');
	}
...
function populateItemStat() {
  ...
  target.append('<button class="dropdown-item" onclick="selectSearchStat();updateSearchResult();"><div class="img-sort a-z"></div></button>');

  for (var key in statList) {
    target.append('<button class="dropdown-item" onclick="selectSearchStat(\'' + statList[key] + '\');updateSearchResult();"><div class="img-sort ' + statList[key] + '"></div></button>');
	}
}
...
function populateResists() {
    ...
    for (var index in elementList) {
        div.append('<div class="col-2 d-flex align-items-center resist ' + elementList[index] + ' ' +  escapeDot("resist|" + elementList[index] + ".percent") + '" data-toggle="tooltip" title="Resist '+elementList[index]+ '">'+
                   '<div class="img-sm img-element ' + elementList[index] + '"></div>'+
                   '<div class="value px-2">0%<div></div>');
    }
    var div = $("#resultStats .resists .ailments");

    for (var index in ailmentList) {
        div.append('<div class="col-2 d-flex align-items-center resist ' + ailmentList[index] + ' ' +  escapeDot("resist|" + ailmentList[index] + ".percent") +'">'+
                   '<div class="img-sm img-ailment ' + ailmentList[index] + '"></div>'+
                   '<div class="value px-2">0%<div></div>');
    }
}
...
let handleExternalControl = function(message) {
    ...
    switch(data.type) {
        ...
        case 'setMonsterElementalResist':
            $('.monster #elementalResists .element.' + data.value.element + ' .elementalResist').val(data.value.value);
